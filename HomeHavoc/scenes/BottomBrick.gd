...
func _integrate_forces(state):
	if connected_to_drone == false && state.get_contact_count() == 1 :
		...
		print("The block is sticking on a ", body_on_which_sticked.get_name())
		var tr_ci_world_to_block = get_global_transform()
		var tr_ci_world_to_collider = body_on_which_sticked.get_global_transform()
		tr_ci_collider_to_block = tr_ci_world_to_collider.inverse() * tr_ci_world_to_block
